...
接收端接口 v1.2 - 双环境支持
功能：支持测试和生产两种运行模式
  - 测试模式: 端口 8888 (本地测试，多用户可共享)
  - 生产模式: 端口 9999 (专用生产环境，避免冲突)

接收包含多个车辆状态字段的数据，并提供回调接口，用户可以自定义数据处理逻辑。
"""
...
import threading
import time
from comm_quality_monitor import CommQualityMonitor


class DataReceiver:
    """数据接收器"""

    def __init__(self, relay_ipv6, relay_port, mode="生产", enable_quality_monitor=True):
        """
        初始化接收器
        :param relay_ipv6: 中转服务器地址 (域名或IPv4/IPv6)
        :param relay_port: 中转服务器端口
        :param mode: 运行模式 ("测试" 或 "生产")
        :param enable_quality_monitor: 是否启用通信质量监测
        """
        ...
        self.quality_monitor = CommQualityMonitor(window_size=100, display_interval=5.0) if enable_quality_monitor else None
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"✓ [{self.mode}模式] 已连接到中转服务器: {self.relay_address[0]}:{self.relay_address[1]}")
            return True
    ...
    def _process_message(self, message):
        ...
        try:
            ...
            # 处理通信质量监测
            if self.enable_quality_monitor and self.quality_monitor:
                self.quality_monitor.process_message(data_dict)

                # 定期显示统计信息
                if self.quality_monitor.should_display():
                    self.quality_monitor.print_statistics()
...
def example_usage_test():
    ...
    receiver = DataReceiver(
        relay_ipv6="43.143.207.201",      # 本地服务器地址
        relay_port=8888,              # 测试环境端口
        mode="测试"
    )
...
def example_usage_quality_test():
    """质量监测测试模式使用示例"""
    print("\n" + "="*60)
    print("  启动质量监测模式接收器 (测试环境)")
    print("="*60)

    # 自定义回调函数：只处理接收到的消息，统计由监测器处理
    def quality_test_handler(data_dict):
        """质量测试处理函数 - 简化的数据显示"""
        seq = data_dict.get('seq', 'N/A')
        vehicle_id = data_dict.get('vehicle_id', 'UNKNOWN')
        actual_speed = data_dict.get('actual_speed_kmh', 'N/A')
        print(f"[Seq: {seq:>6}] {vehicle_id:<20} | Speed: {actual_speed:>7.2f} km/h")
    ...
    # 设置自定义回调函数
    receiver.set_callback(quality_test_handler)
...
def example_usage_prod():
    ...
    # 创建生产模式接收器实例
    receiver = DataReceiver(
        relay_ipv6="43.143.207.201",  # 生产服务器地址
        relay_port=9999,               # 生产环境端口
        mode="生产"
    )
...
def example_usage():
    """使用示例 - 可选择测试或生产模式"""
    print("\n请选择运行模式:")
    print("  1 - 测试模式 (端口 8888, 本地服务器)")
    print("  2 - 生产模式 (端口 9999, 生产服务器)")
    print("  3 - 质量监测模式 (端口 8888, 带通信质量监测)")
    ...
    if choice == "1":
        ...
        example_usage_quality_test()
    else:
        print("无效的选择，默认启动生产模式。")
        example_usage_prod()
