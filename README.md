...
- 💼 **实习经历**：腾讯科技（深圳）有限公司（视觉算法实习生） | 九号公司-未岚大陆（视觉感知算法实习生）| 香港铁路有限公司（Engineering Intern）
- 📫 **联系我**：24120172@bjtu.edu.cn | 188-1075-7598
- ⚡ **一句话自评**：项目实践经历丰富，擅长技术研发，产品写作与逻辑沟通，自驱力MAX，沟通力在线！
...
### 🧑‍💻 **腾讯科技（深圳）有限公司** `2026.04—至今`
*视频算法实习生（TRTC 音视频 SDK · 终端画质增强）*

- **技术栈**：`SR-LUT` `SPAN` `BasicSR` `超分辨率` `图像增强` `GAN` `端侧部署` `PyTorch`
- **项目描述**：为 TRTC 实时音视频「至臻效果」研发端侧人脸画质增强算法，让直播/视频通话在中低端设备上更清晰
- **核心贡献**：
  - 1.面向直播、视频通话等人像场景，基于SR-LUT 构建轻量级人脸超分与增强pipeline，完成模型训练、推理、评估全流程，提升中低端 Android 设备上行人脸增强算法覆盖率，并为端侧架构选型提供认知依据；
  - 2.调研并横向对比SPAN、RLFN 等NTIRE 系列轻量超分/增强模型并输出横向对比报告，以SPAN 替换现网模型Bicubic，在Qalign 与主观观感持平下使GFLOPs 降低约30%，针对黑框眼镜闪烁伪影，从门控、损失权重多维度优化；
  - 3.针对头部客户高美颜直播流，定位训练-测试域分布偏移，设计分区监督退化策略（皮肤磨皮/五官定向模糊）优化效果；系统梳理训练数据、退化策略、损失函数、模型结构等对磨皮/锐度等效果的影响，形成可复用调参指南，支撑多客户快速迭代。
- **关键问题与解决方案**：
  - 1.现网模型算力偏高、难以覆盖中低端设备 → 引入轻量模型 SPAN 替换 Bicubic，在 Qalign 与主观观感持平的前提下显著压缩 GFLOPs 与推理耗时；
  - 2.训练数据与真实端侧场景分布不一致（测试集好、上设备差）→ 定位训练-测试域分布偏移问题，设计分区监督退化策略进行优化；构建数据闭环，以真机直采 YUV 为 LR、大模型增强帧为 HR 构建真实数据集，跳过反向退化建模，使离线指标与端侧实测更一致；
  - 3.黑框眼镜+强光场景高频闪烁伪影 → 从乘性门控软化、全局残差、下采样抗混叠、GAN/感知损失权重再平衡等多维度做消融实验定位与优化。
- **关键成果**：
  - 性能指标：以 SPAN 替换现网 Android/iOS 上行 Bicubic，模型计算量（GFLOPs）降低约 30%，推理耗时同步下降，Qalign 与人眼主观观感基本持平；黑框眼镜强光场景高频闪烁得到有效缓解；
  - 方法沉淀：产出轻量增强模型横向对比调研报告与可复用调参指南，覆盖 SPAN/SPANF/ECBSR/RLFN/SAFMN 等多模型储备，梳理训练数据集、退化策略、损失函数、模型架构对模型效果的影响，形成多客户快速迭代能力，为后续版本模型迭代提供技术支撑；
  - 业务影响：提升中低端设备上行人脸增强的覆盖率与复杂场景鲁棒性，直接支撑 TRTC「至臻效果」在秀场直播、电商带货等核心场景的画质体验，并为后续模型迭代提供技术储备。
- **项目展示**：由于涉及到公司保密数据，不便进行展示。

---

### 🤖 **未岚大陆科技股份有限公司** `2025.09—2026.01`
*视觉算法实习生*

- **技术栈**：`MobileNet` `HRNet` `语义分割` `数据增强` `ROS`
- **项目描述**：构建割草机场景语义分割pipeline，让机器人分清草坪和障碍物
- **核心贡献**：
  - 1.基于MobileNet/HRNet构建割草机场景语义分割pipeline，完成cooper产品线感知分割算法的研发与落地；
  - 2.打通从数据拉取、推断到模型迭代的全流程链路，构建并维护数据闭环系统；
  - 3.与路测、产品团队协作定位Corner Case，与标注团队配合优化数据质量，提升研发效率。
- **关键问题与解决方案**：
  - 1.复杂场景感知鲁棒性差（如台阶误识别）→ 设计数据-模型协同优化方案，通过构建专项数据集与引入辅助分割头，使台阶识别精度显著提升；
  - 2.模型迭代效率低（2周/次）→ 构建自动化难例挖掘与增量训练数据闭环，打通云端数据筛选至模型迭代链路，使迭代周期缩短50%；
  - 3.感知延迟导致避障卡顿 → 调整模型输出分辨率（288x512→256x256），在保证精度的前提下，降低推理延迟
- **关键成果**：
  - 性能指标：模型迭代效率提升一倍；推理延迟平均降低15%；**草坪/障碍物分割精度（mIoU）提升3%**；
  - 业务影响：显著提升复杂场景下感知鲁棒性，有效减少因感知问题导致的用户反映；优化的感知模块直接支撑了路径规划与避障决策，保障了Cooper作为高性价比主打产品的核心体验
- **项目展示**：由于涉及到公司保密数据，不便进行展示。

---

### 🚇 **香港铁路有限公司（MTR）- Engineering Intern** `2024.06—2025.07`
...
### 🌉 **自主运行与编组控制无人车** `2024.10—2026.01`

- **技术栈**：`激光雷达` `IMU` `RTK` `多传感器融合`
- **项目描述**：面向校园场景的轨道交通自主运行技术验证需求，研发从单车智能到多车协同的全栈式无人车系统。主要包括四个方面的工作：
- （1）多传感器融合定位与高精度地图构建；
- （2）车地协同通信与数据平台开发；
- （3）ATO规划与决策算法集成；（4）双车编组运行与动态解编/重编功能验证。
- **核心贡献**：
- 多传感器融合定位系统研发：参与设计基于3D激光雷达、高频率IMU、RTK实时动态定位的融合定位方案。在GPS信号良好路段，利用RTK获取厘米级定位数据；在信号遮挡或隧道区域，通过激光雷达点云匹配与IMU预积分实现连续高精度定位，保障车辆全路段稳定运行。
- 高精度地图构建：基于车载顶部全向激光雷达与两侧补盲雷达，部署点云建图算法。通过角点/面点特征提取、帧间配准及IMU融合优化，生成行驶路段的高精度点云地图，为车辆定位与规划提供基准。

📂 **项目代码**：👉 [`Lidar_mapping`](https://github.com/Kevin-LI-02/Haojie-Kevin-LI/tree/main/Lidar_mapping) 
包含：地图制作与显示、地图切换、通信测试
...
| 论文标题 | 发表情况 | 角色 |
|:---------|:---------|:-----|
| HAIRS: A Perception-Retrieval Fusion Framework for Real-time Railway Intelligent Decision Support | 发表至2026 IEEE ITSC<br>*(交通领域国际顶会)* | **第一作者** |
...
| 类型 | 名称 | 状态/编号 | 角色 |
|:-----|:-----|:----------|:-----|
| 发明专利 | 一种基于图像识别与人工智能的轨道交通监控及决策方法及装置 | 已授权 | 第二发明人 |
