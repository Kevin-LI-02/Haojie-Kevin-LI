...
- **核心贡献**：
  - 1.基于 SR-LUT 构建人脸场景上行超分/增强 pipeline，完成模型训练、推理、评估全流程，提升中低端 Android 设备上行人脸增强算法覆盖率；
  - 2.构建真机数据闭环：Android 真机直采 YUV，配合 GFPGAN/CodeFormer 等大模型生成 Ground Truth，建立贴近真实场景的数据集并明确优化上限；
  - 3.调研 NTIRE 系列轻量超分/增强模型并输出横向对比报告，与团队协作推进 SPAN/ECBSR等模型 替换现网 Bicubic 的落地验证。
- **关键问题与解决方案**：
  - 1.现网模型算力偏高、难以覆盖中低端设备 → 引入轻量模型 SPAN 替换 Bicubic，在 Qalign 与主观观感持平的前提下显著压缩 GFLOPs 与推理耗时；
  - 2.训练数据与真实端侧场景分布不一致（测试集好、上设备差）→ 以真机直采 YUV 为 LR、大模型增强帧为 HR 构建真实数据集，跳过反向退化建模，使离线指标与端侧实测更一致；
  - 3.黑框眼镜+强光场景高频闪烁伪影 → 从乘性门控软化、全局残差、下采样抗混叠、GAN/感知损失权重再平衡等多维度做消融实验定位与优化。
- **关键成果**：
  - 性能指标：以 SPAN 替换现网 Android/iOS 上行 Bicubic，模型计算量（GFLOPs）降低约 30%，推理耗时同步下降，Qalign 与人眼主观观感基本持平；
